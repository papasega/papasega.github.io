...
Cette équation simple indique ce qui suit:  
 ...
 - nous supposons que nous avons \textbf{$n$} observations d'un ensemble de données, et nous choisissons sur $i^{ème}$
 - $Y_i$ est la sortie de cette observation appelée la cible
 - $X_i$ est appelé une caractéristique et est une variable indépendante que nous observons
 - $f$ est le vrai modèle qui établit le lien entre les caractéristiques et la sortie
 -  $\epsilon$ est le bruit du modèle. Les données que nous observons ne se situent généralement pas sur une ligne droite, car il existe des variations de la mesure dans la vie réelle.
