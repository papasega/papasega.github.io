...
= p(sama \| •) p(neexal \|  sama) p(lijaasa \|  neexalu) p(bi \|  lijaasa) p(• \|  bi ) 

<img src="https://latex.codecogs.com/svg.latex?\Large&space; p(sama \ \ neexalu \ \ lijaasa \ \ bi) 
= p(sama |  \alpha) p(neexal | sama) p(lijaasa |  neexalu) p(bi | lijaasa) p(\alpha |  bi )  " title="  " />

où • est une variable libre.
