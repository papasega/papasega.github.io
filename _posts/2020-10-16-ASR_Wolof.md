...
Pour notre exemle : Na ngen deef, dalal ak jàmm si reewu terannga bi. 
...
**L'équation de Wintten-Bell**

<img src="https://latex.codecogs.com/svg.latex?\Large&space; P_{WB}(w_n|w^{n-1}_{n-N+1}) = 
\lambda_{w^{n-1}_{n-N+1}}P(w_n|w^{n-1}_{n-N+1}) + (1- \lambda_{w^{n-1}_{n-N+1}})P_{WB}(w_n|w^{n-1}_{n-N+2}) " title="  " /> 
...
Pour calculer les paramètres d'ajustement 𝜆 on se base sur le nombre de mots distincts qui suivent un historique <img src="https://latex.codecogs.com/svg.latex?\Large&space; w^{n-1}_{n-N+1}" title=" " /> tel que:
 ...
 <img src="https://latex.codecogs.com/svg.latex?\Large&space; N_{1+}(w^{n-1}_{n-N+1} •) = card{w_n:C(w_n|w^{n-1}_{n-N+1})>0}" title=" " />  
 où

 <img src="https://latex.codecogs.com/svg.latex?\Large&space; N_{1+}(w^{n-1}_{n-N+1} •)" title=" " /> 
 ...
 avec • est une variable libre. Pour calculer les 
 ...
 <img src="https://latex.codecogs.com/svg.latex?\Large&space; 1- \lambda_{w^{n-1}_{n-N+1}} = \frac{N_{1+}(w^{n-1}_{n-N+1} •)}{N_{1+}(w^{n-1}_{n-N+1} •) + C(w_n|w^{n-1}_{n-N+1}) }" title=" " /> 
 ...
 <img src="https://latex.codecogs.com/svg.latex?\Large&space; P_{WB}(w_n|w^{n-1}_{n-N+1}) = \frac{C(w_n|w^{n-1}_{n-N+1}) + N_{1+}(w^{n-1}_{n-N+1} •)P_{WB}(w_n|w^{n-1}_{n-N+2})}{N_{1+}(w^{n-1}_{n-N+1} •) + C(w_n|w^{n-1}_{n-N+1})}" title=" " /> 

 Le lissage de Witten-Bell accordera ainsi plus de poinds à un N-gramme déjà vus et sinon, **sera en mesure d'estimer la probabilité de renconter une nouvelle N-gramme**. 
